...
<div>
	...
	<div class="existing-webhooks">
		{foreach $webhooks as $webhook key=webhook_name}
			{if $webhook_name}
				<form class="pkp_form existing-webhook" id="{$webhook_name}SettingsForm" method="post"
					action="{url router=$smarty.const.ROUTE_COMPONENT op="manage" category="generic" plugin=$pluginName verb="settings" save=true}">
					{csrf}
					{include file="controllers/notification/inPlaceNotification.tpl" notificationId="{$webhook_name}SettingsFormNotification"}
					{fbvFormArea id="{$webhook_name}SettingsFormArea"}
					...
					<div class="options-row">
						...
						label="plugins.generic.fixssl.manager.settings.submissionStatus"
						checked={$webhook.submissionStatus}}
					</div>
					...
					{fbvFormButtons hideCancel=true submitText="plugins.generic.fixssl.manager.settings.saveWebhook"}
